...
query = {
    ...
    "aggs": {
        "2": {
            "terms": {"field": "name.keyword", "size": 5000, "order": {"_key": "desc"}},
            "aggs": {
                "1": {
                    "top_hits": {
                        "_source": ["value"],
...
def messageReceived(destination, message, headers):
    ...
    if (filename != None) and (len(mes) > 10):
        try:
            ...
            log_message(
                "Import of file [%s] failed. Duration: %d Exception: %s."
                % (headers["file"], (endtime - starttime), str(e))
            )
    ...
    try:
        log_message(
            "Import of file [%s] finished. Duration: %d Records: %d."
            % (headers["file"], (endtime - starttime), df.shape[0])
        )
    except:
        log_message(
            "Import of file [%s] finished. Duration: %d."
...
def decodeMetaData(fullpath):
    ...
    logger.info("<<< decodeMetaData >>>")
    logger.info(f"site, contract: {site}, {contract}")
    namecols = fullpath.split("/")
    logger.info(f"namecols: {namecols}")
    lennamecols = len(namecols)
    logger.info(f"lennamecols: {lennamecols}")
    if lennamecols >= 2:
...
def computeType(metername):
    ...
    if (metername.lower().find("eau") >= 0) or (metername.lower().find("water") >= 0):
        return "water"
    if (
        (metername.lower().find("tension") >= 0)
        or (metername.lower().find("eclair") >= 0)
        or (metername.lower().find("venti") >= 0)
        or (metername.lower().find("elec") >= 0)
    ):
        return "elec"
    if (metername.lower().find("photo") >= 0) or (
        metername.lower().find("zonnepanelen") >= 0
    ):
        return "photo"
...
def cleanData(filename, mes):
    ...
    if contract == "COGLTS":
        ...
        DebitBiogazThiopaq = dfindexed.loc[
            "COGLTS_BIOLTS_Valeur_Debit_Biogaz_Thiopaq"
        ].at[3]
        DebitBiogazCogen = dfindexed.loc["COGLTS_BIOLTS_Valeur_Debit_Biogaz_Cogen"].at[
            3
        ]
        ...
        fctThiopaqCond1 = (
            1
            if dfindexed.loc["COGLTS_BIOLTS_Valeur_Debit_Biogaz_Thiopaq"].at[3] > 120
            else 0
        )
        fctThiopaqCond2 = (
            1
            if (
                dfindexed.loc["COGLTS_BIOLTS_Valeur_Pression_Thiopaq_Entree"].at[3] > 15
                and dfindexed.loc["COGLTS_BIOLTS_Valeur_Pression_Thiopaq_Entree"].at[3]
                < 36
            )
            else 0
        )
        fctThiopaqCond3 = dfindexed.loc["LUTOSA_Etat_Autor_Biog"].at[3]
        fctThiopaqCondGlobal = (
            1 if fctThiopaqCond1 + fctThiopaqCond2 + fctThiopaqCond3 == 3 else 0
        )

        consoDispos = (
            dfindexed.loc["LUTOSA_Etat_Peleur1_Fct"].at[3]
            + dfindexed.loc["LUTOSA_Etat_Peleur2_Fct"].at[3]
            + dfindexed.loc["LUTOSA_Etat_Blancheur_1"].at[3]
            + dfindexed.loc["LUTOSA_Etat_Blancheur_2"].at[3]
        )
        fctCogenCond1 = 1 if consoDispos >= 2 else 0
        fctCogenCond2 = (
            1 if dfindexed.loc["LUTOSA_TempRetour_Boucle_HT"].at[3] <= 80 else 0
        )
        fctCogenCond3 = (
            1
            if dfindexed.loc["COGLTS_BIOLTS_Valeur_Debit_Biogaz_Thiopaq"].at[3] > 220
            else 0
        )
        ...
        data = [
            ...
            {
                0: "LUTOSA_Cond_Fct_Global_Thiopaq",
                1: datefile,
                2: param1,
                3: fctThiopaqCondGlobal,
                4: param2,
            },
            {
                0: "LUTOSA_Cond_Fct_1_Cogen",
                1: datefile,
                2: param1,
                3: fctCogenCond1,
                4: param2,
            ...
            {
                0: "LUTOSA_Cond_Fct_Global_Cogen",
                1: datefile,
                2: param1,
                3: fctCogenCondGlobal,
                4: param2,
            },
        ]
...
def handleOneMessage(name, body):
    ...
    if decodeMetaData(name) == 0:
        ...
        for index, row in df.iterrows():
            ...
            if row[0] in lastvaluecache:
                if lastvaluecache[row[0]]["key"] != key:
                    ...
                    newrec = {
                        "src": "gtc",
                        "client_area_name": client_area_name,
                        "type": computeType(row[0]),
                        "area_name": area_name,
                        "client": contract,
                        "area": site,
                        "name": row[0],
                        "@timestamp": ts,
                        "date": localdate.strftime("%Y-%m-%d %H:%M:00"),
                        "value": row[3],
                        "value_15": cacheres[0],
                        "value_day": cacheres[1],
                    }
                    ...
                    id = (
                        client_area_name
                        + utc_to_local(row["date"]).strftime("%Y%m%d%H%M")
                    ).lower()
        ...
        obj = {"start_ts": int(first_alarm_ts), "area_name": area_name}
        # logger.info(obj)
...
if __name__ == "__main__":
    ...
    res = es.search(index="opt_*", size=0, query=query["query"], aggs=query["aggs"])
    ...
    try:
        ...
        for tag in data:
            ...
            timestamp = datetime.fromtimestamp(
                int(tag["1"]["hits"]["hits"][0]["sort"][0] / 1000)
            )
